import React from 'react';

const Packages = () => {
  return (
    <section className="max-w-7xl mx-auto px-4 md:px-0 py-6" id="packages">
      <h2 className=" text-4xl text-center font-light text-navy mb-4">
        Tutoring <span className="font-extrabold">Packages</span>
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div className="bg-lightBrown rounded text-navy flex flex-col space-y-4 items-center justify-around px-3 py-4">
          <h3 className="font-extrabold text-lg">Package 1</h3>

          <p className="font-medium">
            Full descriptionEquiscient velignat magnatur, core re lia
            nectaquatio. Nam, conem comnien dictiis prat molorem sitis ma
            porionsed ea qui omnim atur, omnia demquibus iuntur mos maionsequat
            alignihil inist, comnit, quas adignis citate soluptatiur alictas
            pidit, incium quist,
          </p>
          <button className="rounded-full w-2/4 bg-navy text-white text-lg py-2 px-3 text-center">
            Sign up
          </button>
        </div>
        <div className="bg-lightBrown rounded text-navy flex flex-col space-y-4 items-center justify-around px-3 py-4">
          <h3 className="font-extrabold text-lg">Package 2</h3>

          <p className="font-medium">
            Full descriptionEquiscient velignat magnatur, core re lia
            nectaquatio. Nam, conem comnien dictiis prat molorem sitis ma
            porionsed ea qui omnim atur, omnia demquibus iuntur mos maionsequat
            alignihil inist, comnit, quas adignis citate soluptatiur alictas
            pidit, incium quist,
          </p>
          <button className="rounded-full w-2/4 bg-navy text-white text-lg py-2 px-3 text-center">
            Sign up
          </button>
        </div>
        <div className="bg-lightBrown rounded text-navy flex flex-col space-y-4 items-center justify-around px-3 py-4">
          <h3 className="font-extrabold text-lg">Package 3</h3>

          <p className="font-medium">
            Full descriptionEquiscient velignat magnatur, core re lia
            nectaquatio. Nam, conem comnien dictiis prat molorem sitis ma
            porionsed ea qui omnim atur, omnia demquibus iuntur mos maionsequat
            alignihil inist, comnit, quas adignis citate soluptatiur alictas
            pidit, incium quist,
          </p>
          <button className="rounded-full w-2/4 bg-navy text-white text-lg py-2 px-3 text-center">
            Sign up
          </button>
        </div>
        <div className="bg-lightBrown rounded text-navy flex flex-col space-y-4 items-center justify-around px-3 py-4">
          <h3 className="font-extrabold text-lg">Package 4</h3>

          <p className="font-medium">
            Full descriptionEquiscient velignat magnatur, core re lia
            nectaquatio. Nam, conem comnien dictiis prat molorem sitis ma
            porionsed ea qui omnim atur, omnia demquibus iuntur mos maionsequat
            alignihil inist, comnit, quas adignis citate soluptatiur alictas
            pidit, incium quist,
          </p>
          <button className="rounded-full w-2/4 bg-navy text-white text-lg py-2 px-3 text-center">
            Sign up
          </button>
        </div>
      </div>
    </section>
  );
};

export default Packages;
